using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public static partial class Constants
{
    #region VFX
    public static readonly string VFX_Player_Parrying = "VFX_Player_Parrying";
    public static readonly string VFX_Player_Guard = "VFX_Player_Guard";
    public static readonly string VFX_Player_Attack = "VFX_Player_Attack";

    public static readonly string VFX_Player_Combo_Attack = "VFX_Player_Combo_Attack";
    public static readonly string VFX_Player_Smash_Attack = "VFX_Player_Smash_Attack";
    public static readonly string VFX_Player_Stinger_Attack = "VFX_Player_Stinger_Attack";

    public static readonly string VFX_Enemy_Flame_Area = "VFX_Enemy_Flame_Area";
    public static readonly string VFX_Enemy_Breath = "VFX_Enemy_Breath";
    
    public static readonly string VFX_Black_Dragon_Storm_Field = "VFX_Black_Dragon_Storm_Field";
    public static readonly string VFX_Black_Dragon_Lightning_Strike = "VFX_Black_Dragon_Lightning_Strike";
    public static readonly string VFX_Black_Dragon_Fire_Ball = "VFX_Black_Dragon_Fire_Ball";
    #endregion

    #region Audio
    public static readonly string Audio_Halberd_Guard_Break = "Audio_Halberd_Guard_Break";
    public static readonly string Audio_Halberd_Parrying = "Audio_Halberd_Parrying";
    public static readonly string Audio_Halberd_Swing_1 = "Audio_Halberd_Swing_1";
    public static readonly string Audio_Halberd_Swing_2 = "Audio_Halberd_Swing_2";
    public static readonly string Audio_Halberd_Swing_3 = "Audio_Halberd_Swing_3";

    public static readonly string Audio_Shield_Guard_Break = "Audio_Shield_Guard_Break";
    public static readonly string Audio_Shield_Parrying = "Audio_Shield_Parrying";
    public static readonly string Audio_Sword_Swing_1 = "Audio_Sword_Swing_1";
    public static readonly string Audio_Sword_Swing_2 = "Audio_Sword_Swing_2";
    public static readonly string Audio_Sword_Swing_3 = "Audio_Sword_Swing_3";
    #endregion

    // Sprite
    public static readonly string Sprite_Thumbnail_Sword_Shield = "Sprite_Thumbnail_Sword_Shield";
    public static readonly string Sprite_Thumbnail_Halberd = "Sprite_Thumbnail_Halberd";

    // Prefab
    public static readonly string Prefab_Event_System = "Prefab_Event_System";

    public static readonly string Prefab_Enemy_Spawner = "Prefab_Enemy_Spawner";
    public static readonly string Prefab_Player_Camera = "Prefab_Player_Camera";
    public static readonly string Prefab_Player_Character = "Prefab_Player_Character";
    public static readonly string Prefab_Player_Character_Slot = "Prefab_Player_Character_Slot";

    public static readonly string Prefab_Floating_Damage_Text = "Prefab_Floating_Damage_Text";

    public static readonly string Prefab_Button_Scene = "Prefab_Button_Scene";
    public static readonly string Prefab_Button_Region = "Prefab_Button_Region";

    public static readonly string Prefab_UI_Scene_Common = "Prefab_UI_Scene_Common";
    public static readonly string Prefab_UI_Scene_Title = "Prefab_UI_Scene_Title";
    public static readonly string Prefab_UI_Scene_Selection = "Prefab_UI_Scene_Selection";
    public static readonly string Prefab_UI_Scene_Game = "Prefab_UI_Scene_Game";


}
