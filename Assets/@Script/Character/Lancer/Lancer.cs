...
public class Lancer : Character
{
    ...
    public override CHARACTER_STATE DetermineCharacterState()
    {
        ...
        if (PlayerInput.IsSpaceKeyDown && CharacterStatus.CurrentStamina >= Constants.CHARACTER_STAMINA_CONSUMPTION_ROLL)
        {
            nextState = CharacterState.CompareStateWeight(nextState, CHARACTER_STATE.ROLL);
        }

        if (PlayerInput.IsRKeyDown && CharacterStatus.CurrentStamina >= Constants.CHARACTER_STAMINA_CONSUMPTION_COUNTER)
        {
