...
public class CharacterStateCombo2 : ICharacterState
{
    private int stateWeight;

    public CharacterStateCombo2()
    {
    ...
    public void Update(BaseCharacter character)
    {
        ...
        if (character.Animator.GetNextAnimatorStateInfo(0).IsName(Constants.ANIMATOR_STATE_NAME_SMASH_2))
            character.SwitchCharacterState(CHARACTER_STATE.Smash_2);

        if (character.Animator.GetNextAnimatorStateInfo(0).IsName(Constants.ANIMATOR_STATE_NAME_COMBO_3))
            character.SwitchCharacterState(CHARACTER_STATE.Combo_3);
