...
public class CharacterStateLightAttack02 : IActionState
{
    ...
    public CharacterStateLightAttack02(BaseCharacter character)
    {
    ...
    public void Update()
    {
        ...
        // -> Smash Attack 2
        if (mouseRightDown && character.State.SetStateByAnimationTimeUpTo(animationNameHash, ACTION_STATE.PLAYER_ATTACK_HEAVY_02, 0.4f))
        {
            return;
        }

        // -> Light Attack 3
        if (mouseLeftDown && character.State.SetStateByAnimationTimeUpTo(animationNameHash, ACTION_STATE.PLAYER_ATTACK_LIGHT_03, 0.4f))
        {
            return;
        }
