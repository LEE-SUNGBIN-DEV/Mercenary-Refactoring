...
public class SwordShieldLightAttack01 : IActionState
{
    ...
    public SwordShieldLightAttack01(PlayerCharacter character)
    {
        ...
        animationNameHash = Constants.ANIMATION_NAME_HASH_SWORD_SHIELD_LIGHT_ATTACK_01;
        mouseLeftDown = false;
    ...
    public void Update()
    {
        ...
        // -> Smash Attack 1
        if (mouseRightDown && character.State.SetStateByAnimationTimeUpTo(animationNameHash, ACTION_STATE.PLAYER_SWORD_SHIELD_ATTACK_HEAVY_01, 0.75f))
        {
            return;
        }

        // -> Light Attack 2
        if (mouseLeftDown && character.State.SetStateByAnimationTimeUpTo(animationNameHash, ACTION_STATE.PLAYER_SWORD_SHIELD_ATTACK_LIGHT_02, 0.75f))
        {
            return;
        }
