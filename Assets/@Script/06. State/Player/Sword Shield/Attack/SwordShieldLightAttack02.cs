...
public class SwordShieldLightAttack02 : IActionState
{
    ...
    public SwordShieldLightAttack02(PlayerCharacter character)
    {
        ...
        animationNameHash = Constants.ANIMATION_NAME_HASH_SWORD_SHIELD_LIGHT_ATTACK_02;
        mouseLeftDown = false;
    ...
    public void Update()
    {
        ...
        // -> Smash Attack 2
        if (mouseRightDown && character.State.SetStateByAnimationTimeUpTo(animationNameHash, ACTION_STATE.PLAYER_SWORD_SHIELD_ATTACK_HEAVY_02, 0.75f))
        {
            return;
        }

        // -> Light Attack 3
        if (mouseLeftDown && character.State.SetStateByAnimationTimeUpTo(animationNameHash, ACTION_STATE.PLAYER_SWORD_SHIELD_ATTACK_LIGHT_03, 0.75f))
        {
            return;
        }
