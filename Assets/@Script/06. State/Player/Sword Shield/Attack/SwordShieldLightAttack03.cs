...
public class SwordShieldLightAttack03 : IActionState
{
    ...
    public SwordShieldLightAttack03(PlayerCharacter character)
    {
        ...
        animationNameHash = Constants.ANIMATION_NAME_HASH_SWORD_SHIELD_LIGHT_ATTACK_03;
        mouseLeftDown = false;
    ...
    public void Update()
    {
        ...
        // -> Smash Attack 3
        if (mouseRightDown && character.State.SetStateByAnimationTimeUpTo(animationNameHash, ACTION_STATE.PLAYER_SWORD_SHIELD_ATTACK_HEAVY_03, 0.75f))
        {
            return;
        }

        // -> Light Attack 4
        if (mouseLeftDown && character.State.SetStateByAnimationTimeUpTo(animationNameHash, ACTION_STATE.PLAYER_SWORD_SHIELD_ATTACK_LIGHT_04, 0.75f))
        {
            return;
        }
