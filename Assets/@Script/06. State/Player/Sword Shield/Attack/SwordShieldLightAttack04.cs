...
public class SwordShieldLightAttack04 : IActionState
{
    ...
    public SwordShieldLightAttack04(PlayerCharacter character)
    {
        ...
        animationNameHash = Constants.ANIMATION_NAME_HASH_SWORD_SHIELD_LIGHT_ATTACK_04;
        mouseLeftDown = false;
    ...
    public void Update()
    {
        ...
        // -> Smash Attack 4
        if (mouseRightDown && character.State.SetStateByAnimationTimeUpTo(animationNameHash, ACTION_STATE.PLAYER_SWORD_SHIELD_ATTACK_HEAVY_04, 0.75f))
        {
            return;
        }

        // -> Light Attack 1
        if (mouseLeftDown && character.State.SetStateByAnimationTimeUpTo(animationNameHash, ACTION_STATE.PLAYER_SWORD_SHIELD_ATTACK_LIGHT_01, 0.75f))
        {
            return;
        }
