...
public class HalberdHeavyAttack04 : IActionState
{
    ...
    public HalberdHeavyAttack04(PlayerCharacter character)
    {
        ...
        animationClipInfo = character.AnimationClipTable["Halberd_Heavy_Attack_04"];
    ...
    public void Enter()
    {
        character.Status.ConsumeStamina(Constants.HALBERD_STAMINA_CONSUMPTION_HEAVY_ATTACK_04);
    ...
    public void Update()
    {
        ...
        // Movement
        if (character.Animator.IsAnimationFrameBetweenTo(animationClipInfo, 54, 70))
    ...
    {
        yield return new WaitUntil(() => character.Animator.IsAnimationFrameUpTo(animationClipInfo, 60) && !character.Animator.IsInTransition((int)ANIMATOR_LAYER.BASE));
        halberd.EnableHalberd(COMBAT_ACTION_TYPE.HALBERD_ATTACK_HEAVY_04);
        character.SFXPlayer.PlaySFX("Audio_Halberd_Swing_04");

        yield return new WaitUntil(() => character.Animator.IsAnimationFrameUpTo(animationClipInfo, 75) && !character.Animator.IsInTransition((int)ANIMATOR_LAYER.BASE));
        halberd.DisableHalberd();
    }
